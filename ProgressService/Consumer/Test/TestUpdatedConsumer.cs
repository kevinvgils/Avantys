    ...
    public class TestUpdatedConsumer : IConsumer<TestUpdated>
    {
        ...
        public async Task Consume(ConsumeContext<TestUpdated> context)
        {
            ...
            try
            {
                ...
                TestUpdated returnedTest = await _TestRepository.UpdateTest(test);
                await _progressService.DeleteProgressAsync(new TestCreated(returnedTest.Id, returnedTest.Module));
